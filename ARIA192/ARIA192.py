...
def ARIA192(eng):
    ...
    if (resource_check != 1):
        ...
        Round_constant_XOR(eng, w1, CK_2, n)
        Round_constant_XOR(eng, w2, CK_3, n)
        Round_constant_XOR(eng, w3, CK_1, n)

    w1 = RoundOdd(eng, w1, w0, vect_a, vect_b, bit, ancilla)
    Round_constant_XOR(eng, w1[64:n], KR, 64)  # 192-128 = 64
    ...
    #### print Ciphertext ####
    if (resource_check != 1):
        print_state(eng, PT, n // 4)  # 8d1470625f59ebacb0e55b534b3e462b
...
def RoundOdd(eng, input, key, vect_a, vect_b, bit, ancilla):
    ...
    input[104:112] = SboxInv(eng, input[104:112])
    ...
    input[72:80] = SboxInv(eng, input[72:80])
    ...
    input[40:48] = SboxInv(eng, input[40:48])
    ...
    input[8:16] = SboxInv(eng, input[8:16])
...
def RoundEven(eng, input, key, vect_a, vect_b, bit, ancilla):
    ...
    ## S-box type2
    input[120:] = SboxInv(eng, input[120:])
    ...
    input[88:96] = SboxInv(eng, input[88:96])
    ...
    input[56:64] = SboxInv(eng, input[56:64])
    ...
    input[24:32] = SboxInv(eng, input[24:32])
